                ...
                @foreach($locales as $key => $locale)
                    ...
                    <!-- Title Field -->
                        <div class="form-group">
                            ...
                            {!! Form::text('name['.$locale->code.']', $carModel->translate($locale->code)['name'], ['class' => 'form-control','maxlength'=>"50", 'autofocus', 'style'=>'direction:'.$locale->direction]) !!}
                        </div>
                    </div>
                @endforeach
            </div>
...
<div class="form-group col-sm-12">
    {!! Form::submit(__('Save'), ['class' => 'btn btn-primary']) !!}
    {!! Form::submit(__('Save And Add More'), ['class' => 'btn btn-primary', 'name'=>'continue']) !!}
    <a href="{!! route('admin.carModels.index') !!}" class="btn btn-default">{{ __('Cancel') }}</a>
</div>